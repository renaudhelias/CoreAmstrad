...
--    https://www.digikey.com/eewiki/pages/viewpage.action?pageId=70189075#PS/2MouseInterface(VHDL)-CodeDownloads
--------------------------------------------------------------------------------
...
ENTITY ps2_mouse IS
	GENERIC(
			clk_freq							:	INTEGER := 4_000_000; --50_000_000;	--system clock frequency in Hz
			ps2_debounce_counter_size	:	INTEGER := 4);--8);				--set such that 2^size/clk_freq = 5us (size = 8 for 50MHz)
	PORT(
			clk				:	IN			STD_LOGIC;								--system clock input
			reset_n			:	IN			STD_LOGIC;								--active low asynchronous reset
			ps2_clk			:	in std_logic;--INOUT		STD_LOGIC;								--clock signal from PS2 mouse
			ps2_data			:	in std_logic; --INOUT		STD_LOGIC;								--data signal from PS2 mouse
...
ARCHITECTURE logic OF ps2_mouse IS
	...
	COMPONENT ps2_transceiver IS
		...
		PORT(
			...
			ps2_clk			:	in std_logic; --INOUT		STD_LOGIC;								--PS/2 port clock signal
			ps2_data			:	in std_logic); --INOUT		STD_LOGIC);								--PS/2 port data signal
	END COMPONENT;
