...
public class LatencesCPC {
    ...
    public void go() {
        ...
        // Si prefix, alors il y a un M1 ensuite => � checker le moment de
        // lecture d'un prefix... il est suppos� en dehors de l'instruction,
        ...
        // INC s
        for (String r1 : ABCDEHL) {
            latence("00" + r1 + "100", 4, null, "INC r");
        }
    ...
    Map<LATENCE_TYPE, List<Integer>> resultatUnDoc = new HashMap<LATENCE_TYPE, List<Integer>>();
    Map<LATENCE_TYPE, List<Integer>> resultatUnTest = new HashMap<LATENCE_TYPE, List<Integer>>();
    {
        for (LATENCE_TYPE l : LATENCE_TYPE.values()) {
            resultat.put(l, new ArrayList<Integer>());
            resultatUnDoc.put(l, new ArrayList<Integer>());
            resultatUnTest.put(l, new ArrayList<Integer>());
        }
    }
        ...
        for (LATENCE_TYPE l : LATENCE_TYPE.values()) {
            if (resultat.get(l).size() == 256) {
                ...
                for (int i : resultat.get(l)) {
                    System.out.println("    " + i + " => '1', -- \"" + toHex(i) + "\"");
                }
                for (int i : resultatUnDoc.get(l)) {
                    System.out.println("    " + i + " => '0', -- \"" + toHex(i) + "\" UNDOCUMENTED");
                }
                for (int i : resultatUnTest.get(l)) {
                    System.out.println("    -- \"" + toHex(i) + "\" NOT TESTED");
                }
